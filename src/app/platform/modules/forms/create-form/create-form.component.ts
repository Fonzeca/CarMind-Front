...
import { FormCreate, Pregunta, Seccion } from 'src/app/platform/interfaces/form';
...
export class CreateFormComponent implements OnInit {
  ...
  validAddSection(){
    const validQ = !this.addSection.preguntas.filter(question=> !question.tipo || !question.descripcion || !this.validQuestionOptions(question) ).length
    return (
      this.addSection.nombre &&
      this.addSection.preguntas.length &&
      validQ
    )
  }
  ...
  validQuestionOptions(question:Pregunta){
   if(question.tipo === 'S1' || question.tipo === 'S2'){
     return question.opciones.length && !question.opciones.filter(data=>!data).length;
   }
   return true;
  }
